...
async function ensureFont(family, weight) {
  ...
  // Strategy 1: Use system font (installed in Docker)
  const systemPath = findSystemFontPath(resolvedFamily, weight);
  if (systemPath) {
    try {
      registerFont(systemPath, {
        family: resolvedFamily,
        weight: numericWeight,
        style: 'normal',
      });
      registeredFonts.add(key);
      console.log(`Registered system font: ${resolvedFamily} ${numericWeight} from ${systemPath}`);
      return resolvedFamily;
    } catch (e) {
      console.error(`Failed to register system font ${systemPath}: ${e.message}`);
    }
  }
